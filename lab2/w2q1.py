...
def bits_to_hex(bits):
    ...
    for i in range(0,len(bits),4) :
        result+=format(int(bits[i:i+4],2),"X")
    return result
...
def decrypt_block(block, keys):
    reverse_keys=keys[::-1]
    return encrypt_block(block, reverse_keys)
def add_padding(msg):
    remainder=len(msg)%8
    padding =8-remainder
...
def encrypt(msg, key):
    ...
    for i in range(0,len(msg),8):
        ...
        encrypted = encrypt_block(block_bits, keys)
        ct+=encrypted
    return ct
def decrypt(ct, key):
    keys=generate_keys(key)
    ...
    for i in range(0,len(ct),64):
        ...
        pt_bits += decrypted
    pt=bits_to_text(pt_bits)
    return remove_padding(pt)

msg = input("Enter the message: ")
key = input("Enter the 8-character key: ")
if len(key) != 8:
    print("ERROR: DES key must contain exactly 8 characters.")
    exit()
...
print("Ciphertext (Hexadecimal):",bits_to_hex(cipher))
